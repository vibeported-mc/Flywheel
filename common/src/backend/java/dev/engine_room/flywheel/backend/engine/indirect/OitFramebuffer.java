...
public class OitFramebuffer {
	...
	public void prepare() {
		...
		GL32.glFramebufferTexture(GL32.GL_FRAMEBUFFER, GL32.GL_DEPTH_ATTACHMENT, depthTextureId(renderTarget), 0);
	}
	...
	public void composite() {
		if (useShaderTransparency()) {
			bindRenderTarget(itemEntityTarget());
		} else {
	...
	private static RenderTarget itemEntityTarget() {
		return ((LevelRendererAccessor) Minecraft.getInstance().levelRenderer).flywheel$targets().itemEntity.get();
	}
